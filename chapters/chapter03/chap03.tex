...
Following the systems biology framework outlined by Colinge et al. (2012) \cite{b4}, we analyzed the PDI network by:
\begin{itemize}
...
Following standard molecular phylogenetic protocols \cite{b23}:
\begin{itemize}
    ...
    \item Tree Construction: We used the Molecular Evolutionary Genetics Analysis (MEGA) software to generate phylogenetic trees. The Neighbor-Joining (NJ) method was employed, with the Kimura 2-parameter model for distance calculation to handle nucleotide substitutions \cite{b37}.
\end{itemize}
